...
China coverage is currently thin and flagged for deeper attention going forward — this desk has explicit instructions (OPERATING-INSTRUCTIONS.md §7.1) to treat China as a first-class daily region rather than folding it into a generic 'Asia' line, and that discipline starts properly with the next run. So far only index-level moves have been logged, with no China-specific catalyst identified for them.
